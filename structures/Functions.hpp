...
public:
    ...
    static auto displayBits(float value, int) -> String {
        return String(value, 0) + "bit";
    }

    static auto parseBits(const String& text) -> float {
        auto clean = text.trim();

        if (clean.endsWithIgnoreCase("bit")) {
            clean = clean.dropLastCharacters(3).trim();
        }
