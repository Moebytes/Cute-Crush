...
auto Processor::processBlock(AudioBuffer<float>& buffer, [[maybe_unused]] MidiBuffer& midiMessages) -> void {
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); sample++) {
        if (ppq > 0.0) {
            ...
            this->parameters.crushLFO.syncPPQ(samplePPQ);
        }

        this->parameters.update();
        
        float mix = this->parameters.mix;
        float sampleRate = this->parameters.sampleRateReduction;
        float bitDepth = this->parameters.bitDepthReduction;
        ...
        bitcrusher.processSample(crushedL, crushedR, sampleRate, bitDepth);

        outputL[sample] = dryL + (crushedL - dryL) * mix;
        outputR[sample] = dryR + (crushedR - dryR) * mix;
    }
